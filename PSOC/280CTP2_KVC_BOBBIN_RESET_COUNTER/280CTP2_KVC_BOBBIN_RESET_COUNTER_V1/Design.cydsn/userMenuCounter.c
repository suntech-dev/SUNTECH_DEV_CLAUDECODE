...
{    
        ...
        case FALSE: // Cliking Check
            {
                ...
                switch(getIndexOfClickedButton(&tc, menu.btn,7))
                {
                    ...
                    case 3: 
                    {
                        ...
                        SaveInternalFlash();          // 추가 2024.05.19
                        RESERVED_OUT_1_Write(0);      // 추가 2024.05.19
                        Buzzer(BUZZER_STOP, 0);       // 추가 2024.05.19
                        g_bTrimCountComplete = FALSE; // 추가 2024.05.20
                    }
                    /*
    ...
    if(g_updateCountMenu)
    {
        ...
        SetButtonText(&menu.btn[2],"%d",/*g_ptrMachineParameter->setTrimCount-*/g_ptrCount->count);   //  순방향  2024.05.19
...
{    
    ...
    BUTTON *btnSet = &menu->btn[0];    
    // SET //////////////////////////////////
    btnSet->image           = NULL;    
    btnSet->font            = Grotesk16x32;
    btnSet->backgroundColor = RED;
    btnSet->foregroundColor = WHITE;
    btnSet->outlineColor    = BLACK;        
    btnSet->align           = TEXT_ALIGN_CENTER;
    btnSet->marginLeftRight = 0;
    btnSet->marginTopBottom =  0;
    btnSet->show            = TRUE;
    btnSet->disable         = FALSE;
    btnSet->ghost           = FALSE;

    btnSet->rect.left = r.left;
    btnSet->rect.right =(r.right-r.left) / 3;
    btnSet->rect.top = r.top;
    btnSet->rect.bottom = r.bottom + g_DEFAULT_BUTTON_HEIGHT;    
    int h_half =  (btnSet->rect.bottom - btnSet->rect.top) / 2;
    btnSet->rect.bottom = btnSet->rect.top + h_half;
    SetButtonText(btnSet,"SET");
    /////////////////
   
    BUTTON *btnReset = &menu->btn[3];
    memcpy(btnReset, btnSet, sizeof(BUTTON));
    btnReset->backgroundColor = BLUE;
    btnReset->rect.top =   btnSet->rect.bottom + 1;
    btnReset->rect.bottom =  r.bottom + g_DEFAULT_BUTTON_HEIGHT;  
    SetButtonText(btnReset,"RESET");  
    ...
    setTrim->rect.left = btnSet->rect.right + 1;
...
//    SetButtonText(curTrim,"%d",g_ptrMachineParameter->curTrimCount); 
    ...
    DrawButton(&menu->btn[0]); // SET
    DrawButton(&menu->btn[3]); // RESET    
//    DrawButton(&menu->btn[1]);    
//    DrawButton(&menu->btn[2]);     
        ...
        case FALSE: // Cliking Check
            {
                ...
                {                  
                    case 3: // for up
                        if      (value+menu.inc[selectIncre] < 10000) //<--- 9999
                        {
...
MENUNODE * menuCreate()
{
    ...
    SetButtonText(&g_TitleBar,"KVC Bobbin Reset");
