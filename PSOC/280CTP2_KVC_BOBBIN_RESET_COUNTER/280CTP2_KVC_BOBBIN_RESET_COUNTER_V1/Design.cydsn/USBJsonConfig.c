...
void USB_CommandProcess()
{
    ...
    switch(g_ConfigMeta.uRequestType)
    {
        ...
        case REQ_WRITE_CONFIG:
        {
            ...
            if(g_ConfigMeta.url[len-1] == '/') g_ConfigMeta.url[len-1] = '\0';  // remove last slash
            
            char *p = strstr(g_ConfigMeta.url,"/"); // ip address 뒤에 경로가 있으면
                        
            if(p != NULL)
            {                
                strcpy(g_ptrServer->path,p);  // 경로만 따로 복사한다.
                *p= '\0'; // g_ptrServer->IP 마지막 뒤에 끝을 알리는 0을 넣는다.
            }
            strcpy(g_ptrServer->IP,g_ConfigMeta.url);
            refineIPAddress(g_ptrServer->IP); // URL이 아닌 문자는 제거한다.
...
void SendConfigData()
{
    ...
    printf_USB("\"serverIP\":\"%s%s\",",g_ptrServer->IP,g_ptrServer->path);
