...
- 버전 식별 규칙: 폴더명 `SMALL_BOARD_SCANNER_{연도}_V{번호}` 기준
...
```
> 분석 버전: SMALL_BOARD_SCANNER_{연도}_V{번호}
...
새 버전 폴더(`SMALL_BOARD_SCANNER_{연도}_V{번호}/`)를 트리 구조에 추가.
...
파일 경로의 버전 폴더명(`SMALL_BOARD_SCANNER_YYYY_VN`)을 새 버전으로 갱신.

---

## `.md` → `.html` 변환 스타일 가이드

`*.md` 파일을 HTML로 변환할 때 **`c:\SUNTECH_DEV_CLAUDECODE\PSOC\SMALL_BOARD_SCANNER\README.html`을 기준 템플릿**으로 삼아 아래 규칙을 일관되게 적용한다.
