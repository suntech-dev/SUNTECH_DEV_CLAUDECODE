# SMALL_BOARD_SCANNER 프로젝트 분석 문서

> 최초 작성: 2026-03-03
> 분석 버전: SMALL_BOARD_SCANNER_2026_V1
> 마지막 빌드: 2024-10-29 (성공)
> 마지막 코드 개선: 2026-03-03

---

## 1. 프로젝트 개요

이 프로젝트는 **Cypress PSoC 4** 기반의 **스캐너 소형 통신 보드** 펌웨어입니다.
...
| 데이터 파싱 및 라우팅 | 바코드 데이터를 파싱해 ST-500과 태블릿으로 각각 전송 |
| 스캔 트리거 명령 중계 | ST-500 Touch OP의 트리거 명령을 2D 스캐너에 전달 |
...
| **코어** | ARM Cortex-M0 |
| **Flash** | 32,768 bytes (사용: 14,310 bytes / 43.7%) |
| **SRAM** | 4,096 bytes (사용: 2,668 bytes / 65.1%) |
| **패키지** | 64-TQFP |
...
│   │   ├── MainFunction.cyacd      # 업로드용 펌웨어 바이너리
...
│                                                         │
│  ┌─────────────┐   ┌─────────────┐   ┌───────────────┐ │
│  │ port_MONITORING│   │   port_OP   │   │    BARCODE    │ │
...
│         │                 │                   │         │
│  ┌──────┴──────┐   ┌──────┴──────┐   ┌───────┴───────┐ │
│  │Barcode_Triger│   │   Counter   │   │Pin_StartBoot  │ │
│  │ (디지털 입력) │   │ (디지털 입력)│   │  (디지털 입력) │ │
│  └─────────────┘   └─────────────┘   └───────────────┘ │
└─────────────────────────────────────────────────────────┘
         │                   │                   │
    ST-500 Touch OP       태블릿 OP           2D 스캐너
    (Monitoring Port)      (OP Port)
```
...
|------|----------|------|-----------|------------|
| `port_MONITORING` | ST-500 Touch OP | 양방향 | 9600 bps | 8N2, 인터럽트 기반 수신 |
| `port_OP` | 태블릿 (Android) | 주로 TX | 9600 bps | 8N2, JSON 포맷 |
...
| `receivedFromBarcode[UART_BUF_SIZE]` | `char[]` | 스캐너 수신 버퍼 (크기: 256) |
| `receivedFromBarcodeCount` | `uint8_t` | 수신 바이트 카운터 (타입 안전) |
| `scanTrigerStart[3]` | `char[]` | 스캐너 트리거 명령 `{0x02, 0xF4, 0x03}` |
| `receivedScanTrigerOrder[UART_BUF_SIZE]` | `char[]` | Touch OP 수신 버퍼 (크기: 256) |
...
| 상수 | 값 | 용도 |
|------|-----|------|
| `UART_BUF_SIZE` | `256u` | UART 수신 버퍼 크기 |
| `TIMEOUT_MS_UART` | `50u` | 수신 완료 판정 타임아웃 (ms) |
| `COUNTER_MIN_MS` | `200u` | Counter 유효 최소 간격 (ms) |
| `COUNTER_MAX_MS` | `1900u` | Counter 유효 최대 간격 (ms) |
| `SCAN_TRIGER_ORDER_STR` | `"$$$$#99900035;%%%%"` | 스캔 트리거 명령 문자열 |
| `SCAN_MODE_CMD_STR` | `"$$$$#99900304;%%%%"` | 스캐너 연결 확인 명령 문자열 |
| `FIRMWARE_VERSION` | `"2D_ST500_MonitoringPort"` | 펌웨어 버전 식별자 |
...
- 항상 `BARCODE_UartGetChar()` 를 먼저 호출하여 RX 버퍼 소비 (무한루프 방지)
- `UART_BUF_SIZE - 1` 초과 시 데이터 폐기 (버퍼 오버플로우 방어)

#### process_MONITORING(void) 함수

ST-500 Touch OP로부터 명령을 수신하고 두 가지 명령을 처리합니다:
1. **트리거 명령** (`SCAN_TRIGER_ORDER_STR`) → 2D 스캐너에 트리거 신호 전송 (`{0x02, 0xF4, 0x03}`)
2. **스캔 모드 조회** (`SCAN_MODE_CMD_STR`) → 스캐너 연결 확인 응답 반환
- 항상 `port_MONITORING_UartGetChar()` 를 먼저 호출하여 RX 버퍼 소비 (무한루프 방지)
- `UART_BUF_SIZE - 1` 초과 시 데이터 폐기 (버퍼 오버플로우 방어)
...
1. Counter 신호 감지 (엣지 감지 방식)
   - 하강 엣지 (1→0): 타이머 초기화
   - 상승 엣지 (0→1): 시간 간격 계산 → COUNTER_MIN_MS~COUNTER_MAX_MS 범위이면 태블릿으로 카운트 JSON 전송

2. 타임아웃 처리 (timerCount - lastReceivedTime > TIMEOUT_MS_UART)
   - Touch OP 수신 버퍼 초기화 (노이즈 데이터 폐기)
   - 스캐너 데이터가 있으면 파싱 후 라우팅:
     * ptrFirst (DesignNo) → NULL 검사 후 ST-500 Touch OP 전송
     * ptrSecond/ptrThird (DesignIDX/Pieces) → NULL 검사 후 태블릿 JSON 전송

3. process_BARCODE() 호출
4. process_MONITORING() 호출
...
```c
initSysTick();
port_MONITORING_Start();   // ST-500 Touch OP UART 시작
port_OP_Start();           // 태블릿 UART 시작
...
| 필드 | 필수여부 | 전송 목적지 |
|------|---------|-----------|
| DesignNo | 필수 | ST-500 Touch OP |
| DesignIDX | 선택 | 태블릿 (JSON) |
| Pieces | 선택 | 태블릿 (JSON) |
...
2. **바코드 스캔** (코드에 주석처리): "SUNTECH" 텍스트 스캔 시 진입 (현재 비활성화)
...
4. `Bootloader/MainFunction.cyacd` 파일 업로드

---

## 8. 발견된 이슈 및 개선 이력

### 8.1 코드 버그 / 잠재적 위험 — ✅ 2026-03-03 전체 수정 완료
...
```
[수정 전 경고]
main.c:45: warning: array subscript has type 'char' [-Wchar-subscripts]   → uint8_t 변경으로 해소
main.c:146: warning: array subscript has type 'char' [-Wchar-subscripts]  → uint8_t 변경으로 해소
main.c:101: warning: unused variable 'count' [-Wunused-variable]          → 변수 제거로 해소
main.c:100: warning: unused variable 'oldBarcodeTriger' [-Wunused-variable] → 변수 제거로 해소
```
...
1. PSoC Creator 실행
2. `SMALL_BOARD_SCANNER_2026_V1/PatternBarcodeMonitoring.cywrk` 워크스페이스 열기
...
|------|-----------|----------|-------|---------|
| port_MONITORING | 9,600 | 8 | None | 2 |
| port_OP | 9,600 | 8 | None | 2 |
...
| 날짜 | 내용 |
|------|------|
| 2018-01-08 | 안도 신형 OP 버전 성공 |
| 2018-01-14 | HWI 초성 신형 OP 성공 |
| 2018-02-07 | 바코드 1~3자리 모두 처리 가능 최종 성공 |
| 2018-02-14 | PWI 초성 신형 OP 성공 |
| 2020-03-10 | Bootloader 적용, 2D Scanner + Touch OP 지원 |
| 2020-03-10 | HWI 전체 적용. Tablet→OP Port, Touch OP→Monitoring Port 구조 확립 |
| 2021-05-28 | VJ/SCI/PWJ 버전 분기. ST-500 전용 버전 기준 |
| 2024-10-29 | 마지막 빌드 성공 확인 (PSoC Creator 4.4) |
| 2026-03-03 | 코드 품질 개선: 타입 안전성(uint8_t), NULL 포인터 방어, 미사용 변수 제거, 상수 정의 분리, 버퍼 오버플로우 방어, Copyright 수정 |
...
| 파일 | 경로 | 설명 |
|------|------|------|
| 핵심 로직 | `SMALL_BOARD_SCANNER_2026_V1/MainFunction.cydsn/main.c` | 메인 루프 및 데이터 처리 |
| 헤더/상수 | `SMALL_BOARD_SCANNER_2026_V1/MainFunction.cydsn/main.h` | 전역 선언, 상수 정의, 버전 |
| 초기화 | `SMALL_BOARD_SCANNER_2026_V1/MainFunction.cydsn/common.c` | 하드웨어 초기화 |
| 타이머 | `SMALL_BOARD_SCANNER_2026_V1/MainFunction.cydsn/sysTick.c` | 1ms SysTick 콜백 |
| 부트로더 main | `SMALL_BOARD_SCANNER_2026_V1/bootloader.cydsn/main.c` | 부트로더 진입점 |
| 업로드 바이너리 | `SMALL_BOARD_SCANNER_2026_V1/Bootloader/MainFunction.cyacd` | UART 업그레이드용 |
| 업로더 도구 | `SMALL_BOARD_SCANNER_2026_V1/Bootloader/UARTBootloaderHost.exe` | PC 업로드 프로그램 |
