...
#include "LCD_RS.h"
    ...
    #define LCD_RS_SetP4PinDriveMode(shift, mode)  \
    do { \
        LCD_RS_PC =   (LCD_RS_PC & \
                                (uint32)(~(uint32)(LCD_RS_DRIVE_MODE_IND_MASK << \
                                (LCD_RS_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (LCD_RS_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define LCD_RS_SetP4PinDriveMode(shift, mode)  \
        do { \
            LCD_RS_USBIO_CTRL_REG = (LCD_RS_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(LCD_RS_DRIVE_MODE_IND_MASK << \
                                    (LCD_RS_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (LCD_RS_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		LCD_RS_SetP4PinDriveMode(LCD_RS__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(LCD_RS_DR & (uint8)(~LCD_RS_MASK));
    drVal = (drVal | ((uint8)(value << LCD_RS_SHIFT) & LCD_RS_MASK));
    LCD_RS_DR = (uint32)drVal;
}
...
{
    return (uint8)((LCD_RS_PS & LCD_RS_MASK) >> LCD_RS_SHIFT);
}
...
{
    return (uint8)((LCD_RS_DR & LCD_RS_MASK) >> LCD_RS_SHIFT);
}
...
void LCD_RS_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  LCD_RS_INTCFG & (uint32)(~(uint32)position);
    LCD_RS_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(LCD_RS_INTSTAT & LCD_RS_MASK);
	LCD_RS_INTSTAT = maskedStatus;
    return maskedStatus >> LCD_RS_SHIFT;
}
