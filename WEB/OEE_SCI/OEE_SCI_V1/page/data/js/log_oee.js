...
/* const columnConfig = [
  ...
  { key: 'work_date', label: 'Work Date', visible: true },
  { key: 'time_update', label: 'Time Update', visible: true },
  { key: 'shift_idx', label: 'Shift', visible: true },
  { key: 'factory_idx', label: 'Factory IDX', visible: true },
  { key: 'factory_name', label: 'Factory Name', visible: true },
  { key: 'line_idx', label: 'Line IDX', visible: true },
  { key: 'line_name', label: 'Line Name', visible: true },
  { key: 'mac', label: 'MAC Address', visible: true },
  { key: 'machine_idx', label: 'Machine IDX', visible: true },
  { key: 'machine_no', label: 'Machine No', visible: true },
  { key: 'design_no', label: 'Design No', visible: true },
  { key: 'process_name', label: 'Process Name', visible: true },
  { key: 'planned_work_time', label: 'Planned Time (sec)', visible: true },
  { key: 'runtime', label: 'Runtime (sec)', visible: true },
  { key: 'productive_runtime', label: 'Productive Runtime', visible: true },
  { key: 'downtime', label: 'Downtime (sec)', visible: true },
  { key: 'availabilty_rate', label: 'Availability (%)', visible: true },
  { key: 'target_line_per_day', label: 'Target Line/Day', visible: true },
  { key: 'target_line_per_hour', label: 'Target Line/Hour', visible: true },
  { key: 'target_mc_per_day', label: 'Target MC/Day', visible: true },
  { key: 'target_mc_per_hour', label: 'Target MC/Hour', visible: true },
  { key: 'cycletime', label: 'Cycle Time', visible: true },
  { key: 'pair_info', label: 'Pair Info', visible: true },
  { key: 'pair_count', label: 'Pair Count', visible: true },
  { key: 'theoritical_output', label: 'Theoretical Output', visible: true },
  { key: 'actual_output', label: 'Actual Output', visible: true },
  { key: 'productivity_rate', label: 'Performance (%)', visible: true },
  { key: 'defective', label: 'Defective', visible: true },
  { key: 'actual_a_grade', label: 'A Grade', visible: true },
  { key: 'quality_rate', label: 'Quality (%)', visible: true },
  { key: 'oee', label: 'OEE (%)', visible: true },
  { key: 'reg_date', label: 'Reg Date', visible: true },
  { key: 'update_date', label: 'Update Date', visible: true },
  { key: 'work_hour', label: 'Work Hour', visible: true }
]; */

document.addEventListener('DOMContentLoaded', async function () {
  console.log('OEE Data Log initialization started');
  ...
  try {
    ...
    console.log('OEE Data Log initialized');
  } catch (error) {
...
window.addEventListener('beforeunload', () => {
  console.log('Log OEE: beforeunload - 페이지 언로드 시작');
...
window.addEventListener('pagehide', () => {
  console.log('Log OEE: pagehide - 페이지 숨김');
...
document.addEventListener('visibilitychange', () => {
  if (document.hidden) {
    console.log('Log OEE: visibilitychange - 페이지 숨김, SSE 연결 종료');
    ...
    console.log('Log OEE: visibilitychange - 페이지 표시');

    // 페이지가 언로드 중이 아닐 때만 재연결
    if (!isPageUnloading && !isTracking && eventSource === null) {
      console.log('Log OEE: 페이지 재표시 - SSE 재연결 시도');
      reconnectAttempts = 0;
      startAutoTracking();
    } else if (isPageUnloading) {
      console.log('⚠️ Log OEE: 페이지 언로드 중 - 재연결 취소');
    }
  }
});
...
window.addEventListener('focus', () => {
  if (isPageUnloading) {
    console.log('Log OEE: 페이지 포커스 - 언로드 플래그 리셋');
...
async function startAutoTracking() {
  ...
  try {
    ...
    const sseUrl = `proc/log_oee_stream.php?${params.toString()}`;
    ...
    if (connectionStatusEl) {
      connectionStatusEl.textContent = 'OEE Data Real-time Connected';
    }
...
function setupSSEEventListeners() {
  ...
  eventSource.addEventListener('connected', function (event) {
    ...
    const connectionStatusEl = document.getElementById('connectionStatus');
    if (connectionStatusEl) {
      connectionStatusEl.textContent = 'OEE data system connected';
  ...
  eventSource.addEventListener('disconnected', function (event) {
    ...
    const connectionStatusEl = document.getElementById('connectionStatus');
    if (connectionStatusEl) {
      connectionStatusEl.textContent = 'OEE Data System Connection Closed';
    }
  });
}
...
function stopTracking() {
  ...
  document.getElementById('connectionStatus').textContent = 'OEE Data System Connection Closed';

  console.log('OEE data streaming stopped.');
}
...
async function attemptReconnection() {
  ...
  if (reconnectAttempts >= maxReconnectAttempts) {
    console.error(`SSE connection continues to fail (${maxReconnectAttempts} attempts). Please check network status or refresh the page.`);
    return;
  }
...
async function handleDateRangeChange(startDate, endDate, label) {
  ...
  console.log(`Date range set to ${startDate.format('YYYY-MM-DD')} ~ ${endDate.format('YYYY-MM-DD')}.`);
...
function exportData() {
  ...
  try {
    ...
    // Create download URL
    const exportUrl = `proc/log_oee_export.php?${params.toString()}`;

    // Open in new window to trigger download
    window.open(exportUrl, '_blank');

    console.log('OEE data log export started. Download will begin shortly.');
  } catch (error) {
...
async function refreshData() {
  ...
  console.log('OEE data has been refreshed.');
}
