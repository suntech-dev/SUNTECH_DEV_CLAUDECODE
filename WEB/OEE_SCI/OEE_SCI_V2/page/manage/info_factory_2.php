...
/**
 * info_factory_2.php — 공장(Factory) 관리 페이지
 *
 * 공장 마스터 데이터 CRUD 페이지 (SAP Fiori 스타일)
 *
 * 사용 모듈:
 *   - resource-manager.js : CRUD, 페이지네이션, 정렬, 필터 통합 관리
 *   - info_factory_2.js   : factoryConfig(컬럼/필터 설정) + 검색/필터/모달 스텝 기능
 *
 * 모달 구조 (2단계):
 *   Step 1: factory_name(필수) + status 선택
 *   Step 2: remark(비고) + 확인 미리보기 (previewName, previewStatus)
 *
 * CSS 로드 순서:
 *   1. fiori-page.css : 공통 SAP Fiori 레이아웃/컴포넌트
 *   2. info_factory_2.css : 공장 관리 페이지 전용 스타일
 */
$page_title = 'Factory Management';
$page_css_files = [
    '../../assets/css/fiori-page.css',
    'css/info_factory_2.css',
];
...
<?php
// 네비게이션 드로어: factory 메뉴 항목을 active 상태로 표시
$nav_active = 'factory';
...
<div class="signage-header">
    ...
    <span class="signage-header__title">Factory Management</span>
    ...
    <div class="signage-header__filters">
        ...
        <div class="search-container">
            <input type="text" id="realTimeSearch" class="fiori-input search-input" placeholder="Search factory...">
            <button class="search-clear" id="searchClear" title="Clear search">&#10005;</button>
        </div>
        <!-- 사용 여부 필터: 기본값 'Y'(Used Only) -->
        <!-- ResourceManager filterConfig에서 statusFilterSelect를 감지하여 데이터 재로드 -->
...
<div id="resourceModal" class="fiori-modal">
    <div class="fiori-modal__content">
        <div class="fiori-card">
            <div class="fiori-card__header">
                <div>
                    <!-- modalTitle: 신규 등록 시 'Add Factory', 수정 시 'Edit Factory' -->
            ...
            <div class="fiori-card__content">
                <form id="resourceForm" class="fiori-form">
                    ...
                    <!-- 스텝 인디케이터: .form-step 요소 + stepIndicator 텍스트 -->
                    ...
                    <!-- Step 1: 기본 정보 입력 -->
                    <!-- data-section="1": showStep()에서 이 섹션만 display:block으로 표시 -->
                    <div class="form-section" data-section="1">
                        ...
                        <div class="fiori-form__group">
                            ...
                            <!-- validateCurrentStep Step1에서 중복 확인 API 호출 -->
                    ...
                    <div class="form-section" data-section="2" style="display: none;">
                        ...
                        <div style="padding: var(--sap-spacing-md); background: var(--sap-surface-2); border-radius: var(--sap-radius-md); margin-top: var(--sap-spacing-md);">
                            ...
                            <div style="font-size: var(--sap-font-size-sm); color: var(--sap-text-secondary);">
                                <!-- previewName: factory_name 입력값 반영 -->
...
<script type="module">
     ...
     *
     * 초기화 순서:
     * 1. createResourceManager(factoryConfig): CRUD/테이블/페이지네이션 초기화
    ...
    import {
        ...
        factoryConfig
    } from './js/info_factory_2.js';

    document.addEventListener('DOMContentLoaded', function() {
        const resourceManager = createResourceManager(factoryConfig);
