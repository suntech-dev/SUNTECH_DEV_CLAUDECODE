...
/**
 * info_downtime_2.php — 다운타임(Downtime) 관리 페이지
 *
 * 다운타임 마스터 데이터 CRUD 페이지 (SAP Fiori 스타일)
 *
 * 사용 모듈:
 *   - resource-manager.js : CRUD, 페이지네이션, 정렬, 필터 통합 관리
 *   - info_downtime_2.js  : downtimeConfig(컬럼/필터 설정) + 검색/필터/모달 스텝 기능
 *
 * 모달 구조 (2단계):
 *   Step 1: downtime_name(필수 + 중복확인) + downtime_shortcut(선택, maxlength=13)
 *   Step 2: status + remark(비고) + 확인 미리보기 (previewName, previewShortcut, previewStatus)
 *
 * CSS 로드 순서:
 *   1. fiori-page.css     : 공통 SAP Fiori 레이아웃/컴포넌트
 *   2. info_downtime_2.css: 다운타임 관리 페이지 전용 스타일
 *
 * 특이사항:
 *   - info_defective_2.php와 동일한 구조 (name + shortcut 2단계 패턴)
 *   - downtime_shortcut: 선택 입력 (optional) — 입력 시에만 2차 중복 확인 API 호출
 *   - status 선택은 Step2에 위치
 */
...
<?php
// 네비게이션 드로어: downtime 메뉴 항목을 active 상태로 표시
$nav_active = 'downtime';
...
<!-- Downtime Modal: 다운타임 등록/수정 다이얼로그 (2단계) -->
...
<div id="resourceModal" class="fiori-modal">
    <div class="fiori-modal__content">
        <div class="fiori-card">
            <div class="fiori-card__header">
                <div>
                    <!-- modalTitle: 신규 등록 시 'Add Downtime', 수정 시 'Edit Downtime' -->
            ...
            <div class="fiori-card__content">
                <form id="resourceForm" class="fiori-form">
                    ...
                    <!-- Step 1: 기본 정보 (다운타임명 + 단축 코드) -->
                    <!-- validateCurrentStep Step1: downtime_name 필수 + 중복 확인 API 1차 호출 -->
                    <!-- downtime_shortcut 입력 시 중복 확인 API 2차 호출 -->
                    <div class="form-section" data-section="1">
                        ...
                        <div class="fiori-form__group">
                            ...
                            <!-- maxlength="13": 단축 코드 최대 13자 제한 (LCD 세로 화면 15자 한계 기준) -->
                            <input type="text" id="downtime_shortcut" name="downtime_shortcut" class="fiori-input"
                                placeholder="Enter shortcut code (optional)" maxlength="13">
                            <div class="fiori-form__help">Optional shortcut code for quick downtime selection (max 13 characters)</div>
                        </div>
                    </div>

                    <!-- Step 2: 추가 정보 (상태 + 비고 + 확인 미리보기) -->
                    <!-- status가 Step2에 위치 (info_defective_2.php와 동일한 구조) -->
                    <div class="form-section" data-section="2" style="display: none;">
                        ...
                        <div style="padding: var(--sap-spacing-md); background: var(--sap-surface-2); border-radius: var(--sap-radius-md); margin-top: var(--sap-spacing-md);">
                            ...
                            <div style="font-size: var(--sap-font-size-sm); color: var(--sap-text-secondary);">
                                <!-- previewName: downtime_name 입력값 반영 -->
                                <div><strong>Downtime name:</strong> <span id="previewName">-</span></div>
                                <!-- previewShortcut: downtime_shortcut 입력값 반영 (없으면 '-') -->
...
<script type="module">
     ...
     *
     * 초기화 순서:
     * 1. createResourceManager(downtimeConfig): CRUD/테이블/페이지네이션 초기화
