...
/**
 * ============================================================
 * 파일명  : log_oee_export_2.php
 * 목  적  : OEE 데이터 로그를 Excel(.xlsx) 파일로 내보내기
 *
 * 주요 기능:
 *  - data_oee 테이블의 전체 컬럼 데이터를 Excel로 출력
...
/**
 * exportOeeDataLog — data_oee 테이블의 OEE 데이터 로그를 Excel로 내보내기
 *
 ...
 *  2. data_oee 테이블에서 info_factory, info_line 조인 후 데이터 조회
...
function exportOeeDataLog(PDO $pdo)
{
    try {
         ...
         *    buildExportFilterParams('do') → 테이블 별칭 'do' 기준으로
         *    factory_idx, line_idx, machine_idx, shift_idx, work_date 필터 적용
         * ---------------------------------------------------------------- */
        $query_conditions = buildExportFilterParams('do');

        /* ----------------------------------------------------------------
         * 2. data_oee 테이블 전체 컬럼 조회 SQL
         *    - info_factory, info_line 테이블과 LEFT JOIN 으로 이름 컬럼 추가
         *    - 최신 작업일(work_date DESC) → 최신 업데이트(update_date DESC) 순 정렬
    ...
    ";
        ...
        /* ----------------------------------------------------------------
         * SQL 실행 및 결과 배열 취득
         *  - PDO::FETCH_ASSOC : 컬럼명을 키로 하는 연관 배열 반환
         * ---------------------------------------------------------------- */
         ...
         * ---------------------------------------------------------------- */
        // Excel 통합 문서(Spreadsheet) 및 활성 시트 생성
        $spreadsheet = new Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();
        $sheet->setTitle('OEE Data Log'); // 시트 탭 이름 설정

        /* ----------------------------------------------------------------
         * 헤더 행 정의 (data_oee 테이블 컬럼 순서와 동일)
         *  - 총 34개 컬럼 (공장명·라인명은 JOIN 컬럼 포함)
         * ---------------------------------------------------------------- */
         ...
         *    예) OEE_Data_Log_2025-01-01_2025-01-31_20250131120000.xlsx
         * ---------------------------------------------------------------- */
