...
/**
 * ============================================================
 * 파일명: data_defective_export_2.php
 * 목  적: 불량(Defective) 데이터를 Excel(.xlsx) 파일로 내보내는 API
 *
 * 주요 기능:
 *  - 공장 → 라인 → 기계 3단계 필터링 지원
...
/**
 * 불량(Defective) 데이터를 Excel로 내보내는 메인 함수
 *
 * @param PDO $pdo PDO 데이터베이스 연결 객체
 */
function exportDefectives(PDO $pdo)
{
    try {
        /*
         * 공통 필터 파라미터 빌더 호출:
         *  - 테이블 alias: 'dd' (data_defective)
        ...
        /*
         * 불량 데이터 조회 SQL
         * - data_defective(dd): 불량 발생 이력 테이블 (메인)
         * - info_factory(f): 공장명 조인
         * - info_line(l): 라인명 조인
         * - elapsed_time 컬럼: 발생 시점부터 현재까지 경과 시간 (h/m/s 포맷)
         */
        $sql = "
      SELECT
        dd.idx,
    ...
    ";
        ...
        // 시트 탭 이름 설정
        $sheet->setTitle('Defective Data');

        /*
         * 헤더 컬럼 정의
         * 순서: NO, ID, Machine No, Factory/Line, Shift, Defective Type,
         *        Status, Occurrence Time, Elapsed Time, Work Date, Updated At
         */
        $headers = [
            'NO',
            'ID',
        ...
        foreach ($data as $index => $row) {
             ...
             *        Defective Type, Status, 발생시각, 경과시간, Work Date, 수정시각
             */
            $sheet->fromArray([
                $index + 1,
                $row['idx'],
         ...
         *  예) defective_data_2026-01-01_2026-01-31_20260101120000.xlsx
         */
        $filename_parts = ['defective_data'];
...
// ── 메인 실행부 ──────────────────────────────────────────────────────────────
// DB 연결 확인 후 exportDefectives() 호출
try {
    if (!$pdo) {
        throw new Exception("Database connection failed");
    }

    exportDefectives($pdo);
} catch (Exception $e) {
    // 치명적 오류: 500 에러 반환
    http_response_code(500);
    error_log("Fatal error in defective export: " . $e->getMessage());
