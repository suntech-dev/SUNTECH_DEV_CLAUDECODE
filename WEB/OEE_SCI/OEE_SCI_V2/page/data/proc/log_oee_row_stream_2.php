...
/**
 * ============================================================
 * 파일명  : log_oee_row_stream_2.php
 * 목  적  : OEE Row(개별 행) 데이터 로그를 SSE(Server-Sent Events)로 실시간 스트리밍
 *
 * 주요 기능:
 *  - data_oee_rows 테이블의 OEE 행 데이터를 SSE로 5초마다 실시간 전송
...
/**
 * getOeeRowDataLog — data_oee_rows 테이블에서 OEE 행 데이터를 조회
 *
 * @param PDO    $pdo       데이터베이스 연결 객체
 * @param string $where_sql WHERE 절 SQL 문자열 (stream_helper.lib.php의 parseFilterParams가 생성)
 * @param array  $params    PDO 바인딩 파라미터 배열
 * @param int    $limit     조회 최대 건수 (기본 1000)
 * @return array            조회된 OEE 행 데이터 연관 배열 목록
 *                          (오류 시 빈 배열 반환)
 */
function getOeeRowDataLog($pdo, $where_sql, $params, $limit = 1000)
{
    ...
    ";
         ...
         * 예) 1 → "Shift 1", 2 → "Shift 2"
         * null 또는 빈 문자열은 그대로 유지 */
...
/**
 * getOeeRowStats — data_oee_rows 테이블의 통계 요약 데이터를 조회
 *
 * 반환 지표:
 ...
 *  - 현재 교대 OEE (getCurrentShiftOeeAvg)
 *  - 전일 OEE (getPreviousDayOeeAvg)
...
function getOeeRowStats($pdo, $where_sql, $params)
{
    try {
         ...
         *  - today_avg_oee  : CURDATE() 당일 데이터만 조건부 평균
         * ---------------------------------------------------------------- */
        $sql = "
      SELECT
...
function startStreaming($pdo)
{
    ...
    /* parseFilterParams('dor', 'work_date', true, '7 DAY')
     *  - 테이블 별칭 'dor' 기준 WHERE 절 생성
...
/**
 * getPreviousDayOeeAvg — 전일(오늘 제외) 평균 OEE 조회
 *
 * 현재 필터 조건을 유지하면서 CURDATE() 이전 날짜 중
 * 가장 최근 작업일 1개의 평균 OEE를 반환.
 * 오늘 데이터가 없는 휴일이어도 그 이전 유효한 날짜의 값 반환 가능.
 *
 * @param PDO    $pdo       데이터베이스 연결 객체
...
/**
 * getCurrentShiftOeeAvg — 현재(또는 선택된) 교대의 평균 OEE 조회
 *
 * 처리 흐름:
 *  1. selected_shift_idx가 있으면 해당 교대의 시간 범위 조회
 *  2. 없으면 findCurrentShift()로 현재 시각 기준 교대 자동 감지
 *  3. 교대의 work_date 및 shift_idx 기준으로 data_oee_rows 테이블 조회
