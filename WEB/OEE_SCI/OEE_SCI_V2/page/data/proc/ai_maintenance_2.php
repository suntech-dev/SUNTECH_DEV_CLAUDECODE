...
/**
 * ============================================================
 * 파일명: ai_maintenance_2.php
 * 목  적: AI 예측 정비(Predictive Maintenance) API
 ...
 *      - A(40%): 런타임 위험도 — 마지막 다운 이후 경과시간 / 평균 고장 간격
...
switch ($date_range) {
    ...
    case '30d': // 최근 30일
        ...
        break;
    default: // today → 7일 기준으로 분석 (오늘 하루만이면 통계 불안정)
...
try {
  ...
  ";
    ...
    // ════════════════════════════════════════════════════════
    // STEP 4. OEE 트렌드 조회 (date_range 기간 평균 OEE)
    //   - avg_oee_7d : 기간 평균 OEE
    ...
    SELECT
      machine_idx,
      AVG(oee)        AS avg_oee_7d,  -- 기간 평균 OEE
      MIN(oee)        AS min_oee_7d,  -- 기간 최소 OEE
      STDDEV(oee)     AS std_oee_7d   -- OEE 표준편차 (불안정도)
    FROM data_oee
