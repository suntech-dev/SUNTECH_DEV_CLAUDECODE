    ...
    try {
      ...
      SELECT
        ...
        CASE
          ...
          ELSE
            CASE
              WHEN TIMESTAMPDIFF(SECOND, da.reg_date, da.update_date) > 0 THEN
                CONCAT(
                  CASE
                    WHEN TIMESTAMPDIFF(HOUR, da.reg_date, da.update_date) > 0
                    THEN CONCAT(TIMESTAMPDIFF(HOUR, da.reg_date, da.update_date), 'h ')
                    ELSE ''
                  END,
                  CASE
                    WHEN TIMESTAMPDIFF(MINUTE, da.reg_date, da.update_date) % 60 > 0
                    THEN CONCAT(TIMESTAMPDIFF(MINUTE, da.reg_date, da.update_date) % 60, 'm ')
                    ELSE ''
                  END,
                  TIMESTAMPDIFF(SECOND, da.reg_date, da.update_date) % 60, 's'
                )
              ELSE '-'
            END
        END as duration
