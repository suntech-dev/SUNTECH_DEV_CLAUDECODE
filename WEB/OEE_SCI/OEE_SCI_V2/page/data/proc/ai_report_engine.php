...
switch ($range) {
  ...
  case '1w':        $from = date('Y-m-d', strtotime('-7 days'));  $to = $today; break;
  case '1m':        $from = date('Y-m-d', strtotime('-30 days')); $to = $today; break;
  case 'custom':
...
// ── 필터 SQL ───────────────────────────────────────────
$fp = []; $fv = [];
if ($factory !== '') { $fp[] = 'do.factory_idx = ?'; $fv[] = $factory; }
if ($line    !== '') { $fp[] = 'do.line_idx = ?';    $fv[] = $line; }
if ($machine !== '') { $fp[] = 'do.machine_idx = ?'; $fv[] = $machine; }
...
try {
  ...
  // ── 2. 다운타임 집계 ────────────────────────────────
  $dt_fp = []; $dt_fv = [$from, $to];
  if ($factory !== '') { $dt_fp[] = 'dd.factory_idx = ?'; $dt_fv[] = $factory; }
  if ($line    !== '') { $dt_fp[] = 'dd.line_idx = ?';    $dt_fv[] = $line; }
  if ($machine !== '') { $dt_fp[] = 'dd.machine_idx = ?'; $dt_fv[] = $machine; }
  ...
  // ── 5. 유지보수 위험도 (Top 5) ─────────────────────
  $mfp = []; $mfv = [];
  if ($factory !== '') { $mfp[] = 'im.factory_idx = ?'; $mfv[] = $factory; }
  if ($line    !== '') { $mfp[] = 'im.line_idx = ?';    $mfv[] = $line; }
  if ($machine !== '') { $mfp[] = 'im.idx = ?';         $mfv[] = $machine; }
  ...
  $ofv = [$from, $to];
  $ofp = [];
  if ($factory !== '') { $ofp[] = 'do.factory_idx = ?'; $ofv[] = $factory; }
  if ($line    !== '') { $ofp[] = 'do.line_idx = ?';    $ofv[] = $line; }
  ...
  foreach ($opt_lines as $ol) {
    $bn = 'availability'; $bn_val = (float)$ol['avg_avail']; $bn_target = 90.0;
    if ((float)$ol['avg_perf']    < $bn_val) { $bn = 'performance'; $bn_val = (float)$ol['avg_perf'];    $bn_target = 90.0; }
    if ((float)$ol['avg_quality'] < $bn_val) { $bn = 'quality';     $bn_val = (float)$ol['avg_quality']; $bn_target = 99.0; }
