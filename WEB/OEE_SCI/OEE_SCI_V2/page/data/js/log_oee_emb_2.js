/**
 * log_oee_emb_2.js — 자수기(EMB) 일별 집계 로그 페이지
 * SSE 대상: proc/log_oee_emb_stream_2.php (data_oee_emb)
 */
...
const columnConfig = [
    ...
    { key: 'work_hour',         label: 'work_hour',         visible: false },
...
function updateLayout() {
    const main = document.getElementById('logEmbMain');
    if (!main) return;
    const rows = [];
    if (!document.getElementById('logEmbStats').classList.contains('hidden')) rows.push('auto');
    if (!document.getElementById('logEmbTable').classList.contains('hidden')) rows.push('1fr');
    rows.push('auto');
    main.style.gridTemplateRows = rows.join(' ');
}

function toggleStatsDisplay() {
    const row = document.getElementById('logEmbStats');
...
function toggleDataDisplay() {
    const rowTable      = document.getElementById('logEmbTable');
    const rowPagination = document.getElementById('logEmbPagination');
...
function initColumnToggle() {
    ...
    columnConfig.forEach(col => {
        ...
        checkbox.addEventListener('change', (e) => { col.visible = e.target.checked; renderTableHeader(); updateTableFromAPI(embData); });
    });
...
function displayEmptyState() {
    ...
    if (tbody) tbody.innerHTML = `<tr><td colspan="${visibleCount}" class="data-table-centered"><div class="fiori-alert fiori-alert--info"><strong>Information:</strong> Loading EMB data log. Please wait...</div></td></tr>`;
}
...
async function startAutoTracking() {
    ...
    eventSource = new EventSource('proc/log_oee_emb_stream_2.php?' + params.toString());
    setupSSEEventListeners();
    isTracking = true;
    const el = document.getElementById('connectionStatus');
    if (el) el.textContent = 'EMB Data Log Real-time Connected';
}
...
function setupSSEEventListeners() {
    ...
    eventSource.addEventListener('connected', function () {
        reconnectAttempts = 0;
        const el = document.getElementById('connectionStatus');
        if (el) el.textContent = 'EMB data log system connected';
    });
    eventSource.addEventListener('emb_data', function (e) {
        const data = JSON.parse(e.data);
        stats   = data.stats;
        embData = data.emb_data;
        updateStatCardsFromAPI(stats);
        updateTableFromAPI(embData);
    ...
    eventSource.addEventListener('disconnected', function () {
        const el = document.getElementById('connectionStatus');
        if (el) el.textContent = 'EMB Data Log System Connection Closed';
    });
}
...
function updateTableFromAPI(list) {
    ...
    if (list.length === 0) {
        const visibleCount = columnConfig.filter(c => c.visible).length;
        tbody.innerHTML = `<tr><td colspan="${visibleCount}" class="data-table-centered"><div class="fiori-alert fiori-alert--info"><strong>Information:</strong> No EMB data matching the selected conditions.</div></td></tr>`;
        renderPagination();
        return;
    }
...
function exportData() {
    if (!embData || embData.length === 0) return;
    try {
        ...
        window.open('proc/log_oee_emb_export_2.php?' + params.toString(), '_blank');
