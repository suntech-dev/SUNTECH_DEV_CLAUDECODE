 ...
 * ============================================================
 * 파일 목적:
 *   OEE(Overall Equipment Effectiveness) 시간별(Hourly) 로그 데이터를
 *   실시간으로 조회하고 테이블로 표시하는 페이지의 핵심 JavaScript.
 *   SSE(Server-Sent Events)를 통해 data_oee_rows_hourly 테이블의
 *   전체 컬럼 데이터를 스트리밍 수신하며, 컬럼 표시/숨김 토글,
 *   페이지네이션, 공장·라인·기계 필터, 날짜 범위 필터,
 *   Excel 내보내기를 지원한다.
 *
 * log_oee_2.js와의 차이점:
 *   - SSE 엔드포인트: log_oee_hourly_stream_2.php
 *   - Export 엔드포인트: log_oee_hourly_export_2.php
 *   - columnConfig에 work_hour 컬럼 추가
 *   - idx 컬럼이 기본 표시(visible: true)
 *   - 레이아웃 ID: logHourlyMain / logHourlyStats / logHourlyTable
 * ============================================================
 */
...
let oeeData = [];                 // 서버에서 수신한 전체 OEE 시간별 데이터 배열
...
// 컬럼 설정 (data_oee_rows_hourly 테이블 전체 컬럼)
// key: DB 컬럼명, label: 헤더 표시명, visible: 초기 표시 여부
const columnConfig = [
    { key: 'idx',                  label: 'idx',                  visible: true  }, // PK (시간별 로그에서는 표시)
    ...
    { key: 'work_hour',            label: 'work_hour',            visible: true  }  // 작업 시간 (시간별 로그 전용)
];
...
function initStickyColumnsScroll() {
    ...
    let _naturalOffsets = [];    // 각 요소의 scrollLeft=0 기준 offsetLeft
    ...
    /**
     * 현재 scrollLeft를 기반으로 각 sticky 컬럼의 translateX를 계산·적용한다.
     */
 ...
 * log_oee_2.js와 다르게 logHourly 접두어 ID를 사용한다.
 */
function updateLayout() {
    const main = document.getElementById('logHourlyMain');
    if (!main) return;

    const rows = [];
    // 통계 섹션이 표시 중이면 auto 추가
    if (!document.getElementById('logHourlyStats').classList.contains('hidden'))  rows.push('auto');
    // 테이블 섹션이 표시 중이면 1fr (남은 공간 모두 차지)
    if (!document.getElementById('logHourlyTable').classList.contains('hidden'))  rows.push('1fr');
...
function toggleStatsDisplay() {
    const row = document.getElementById('logHourlyStats');
...
function toggleDataDisplay() {
    const rowTable      = document.getElementById('logHourlyTable');
    const rowPagination = document.getElementById('logHourlyPagination');
...
/**
 * 활성 공장 목록을 fetch하여 공장 필터 셀렉트박스를 구성한다.
 * 공장 선택 변경 시 기계 목록 초기화 → 라인 연계 갱신 → SSE 재시작
 */
...
/**
 * 활성 라인 목록을 fetch하여 라인 필터 셀렉트박스를 구성한다.
 * 라인 선택 변경 시 기계 목록 연계 갱신 → SSE 재시작
 */
...
/**
 * 전체 기계 목록을 fetch하여 기계 필터 셀렉트박스를 구성한다.
 */
...
/**
 * 공장 ID 변경 시 라인 필터 셀렉트박스를 동적으로 갱신한다.
 * @param {string} factoryId - 선택된 공장 idx
 */
...
/**
 * 공장/라인 ID 변경 시 기계 필터 셀렉트박스를 동적으로 갱신한다.
 * @param {string} factoryId - 선택된 공장 idx
 * @param {string} lineId    - 선택된 라인 idx
 */
...
function initColumnToggle() {
    ...
    columnConfig.forEach(col => {
        ...
        // 체크박스 변경: columnConfig 업데이트 → 테이블 즉시 재렌더
...
function displayEmptyState() {
    ...
    const visibleCount = columnConfig.filter(c => c.visible).length; // 표시 중인 컬럼 수
...
/**
 * SSE 연결을 시작하여 시간별 OEE 로그 데이터를 실시간 수신한다.
 * 엔드포인트: proc/log_oee_hourly_stream_2.php
 */
async function startAutoTracking() {
    if (isTracking) return; // 중복 연결 방지
    const params = new URLSearchParams(getFilterParams());
    // 시간별 SSE 스트림 엔드포인트에 연결
    eventSource   = new EventSource('proc/log_oee_hourly_stream_2.php?' + params.toString());
    setupSSEEventListeners(); // SSE 이벤트 핸들러 등록
    isTracking = true;
    const el = document.getElementById('connectionStatus');
    if (el) el.textContent = 'OEE Hourly Data Real-time Connected';
}
...
function setupSSEEventListeners() {
    ...
    // 'connected' 이벤트: 연결 성공, 재연결 카운터 초기화
    eventSource.addEventListener('connected', function (e) {
        reconnectAttempts = 0;
        const el = document.getElementById('connectionStatus');
        if (el) el.textContent = 'OEE hourly data system connected';
    });

    // 'oee_data' 이벤트: 시간별 OEE 데이터 수신 및 UI 업데이트
    eventSource.addEventListener('oee_data', function (e) {
        const data = JSON.parse(e.data); // JSON 파싱
        stats   = data.stats;            // 통계 데이터 저장
        oeeData = data.oee_data;         // 시간별 OEE 데이터 저장
    ...
    eventSource.addEventListener('disconnected', function () {
        const el = document.getElementById('connectionStatus');
        if (el) el.textContent = 'OEE Hourly Data System Connection Closed';
    });
}
...
/**
 * 시간별 OEE 데이터 배열을 받아 현재 페이지에 해당하는 데이터만
 * 테이블 tbody에 렌더링한다.
 * OEE/가용률/성능률/품질률 컬럼은 SAP Fiori 배지로 표시한다.
 * @param {Array} list - 전체 시간별 OEE 데이터 배열
 */
    ...
    if (list.length === 0) {
        ...
        tbody.innerHTML = `
            <tr>
                <td colspan="${visibleCount}" class="data-table-centered">
                    <div class="fiori-alert fiori-alert--info">
                        <strong>Information:</strong> No OEE hourly data matching the selected conditions.
                    </div>
                </td>
            </tr>`;
        renderPagination();
        ...
        columnConfig.forEach(col => {
            ...
            if (['oee', 'availabilty_rate', 'productivity_rate', 'quality_rate'].includes(col.key) && value !== '-') {
                ...
                if (col.key === 'oee') {
                    ...
                    badgeClass += ' fiori-badge--success'; // 가용률
                } else if (col.key === 'productivity_rate') {
                    badgeClass += ' fiori-badge--info';    // 성능률
                } else if (col.key === 'quality_rate') {
                    badgeClass += ' fiori-badge--warning'; // 품질률
                }
...
// ─── Export / Refresh ─────────────────────────────────────
/**
 * 현재 필터 조건으로 시간별 OEE 로그 데이터를 Excel 형태로 내보낸다.
 * 엔드포인트: proc/log_oee_hourly_export_2.php
 */
function exportData() {
    ...
    try {
        ...
        // 시간별 Export PHP를 새 탭에서 열기
        window.open('proc/log_oee_hourly_export_2.php?' + params.toString(), '_blank');
    } catch (e) {
