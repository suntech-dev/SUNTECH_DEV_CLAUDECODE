// OEE Row Data Log JavaScript v1.0
...
// 컬럼 설정 (data_oee_rows 테이블의 모든 컬럼)
const columnConfig = [
...
document.addEventListener('DOMContentLoaded', async function () {
  console.log('OEE Row Data Log initialization started');
  ...
  try {
    ...
    console.log('OEE Row Data Log initialized');
  } catch (error) {
...
window.addEventListener('beforeunload', () => {
  console.log('Log OEE Row: beforeunload - 페이지 언로드 시작');
...
window.addEventListener('pagehide', () => {
  console.log('Log OEE Row: pagehide - 페이지 숨김');
...
document.addEventListener('visibilitychange', () => {
  if (document.hidden) {
    console.log('Log OEE Row: visibilitychange - 페이지 숨김, SSE 연결 종료');
    ...
    console.log('Log OEE Row: visibilitychange - 페이지 표시');

    // 페이지가 언로드 중이 아닐 때만 재연결
    if (!isPageUnloading && !isTracking && eventSource === null) {
      console.log('Log OEE Row: 페이지 재표시 - SSE 재연결 시도');
      reconnectAttempts = 0;
      startAutoTracking();
    } else if (isPageUnloading) {
      console.log('⚠️ Log OEE Row: 페이지 언로드 중 - 재연결 취소');
    }
  }
});
...
window.addEventListener('focus', () => {
  if (isPageUnloading) {
    console.log('Log OEE Row: 페이지 포커스 - 언로드 플래그 리셋');
    ...
    <tr>
      <td colspan="${visibleColCount}" class="data-table-centered">
        <div class="fiori-alert fiori-alert--info">
          <strong>ℹ️ Information:</strong> Loading OEE row data log. Please wait...
        </div>
      </td>
    </tr>
  `;
...
async function startAutoTracking() {
  ...
  try {
    ...
    const sseUrl = `proc/log_oee_row_stream.php?${params.toString()}`;
    ...
    if (connectionStatusEl) {
      connectionStatusEl.textContent = 'OEE Row Data Real-time Connected';
    }
...
function setupSSEEventListeners() {
  ...
  eventSource.addEventListener('connected', function (event) {
    ...
    const connectionStatusEl = document.getElementById('connectionStatus');
    if (connectionStatusEl) {
      connectionStatusEl.textContent = 'OEE row data system connected';
  ...
  eventSource.addEventListener('disconnected', function (event) {
    ...
    const connectionStatusEl = document.getElementById('connectionStatus');
    if (connectionStatusEl) {
      connectionStatusEl.textContent = 'OEE Row Data System Connection Closed';
    }
  });
}
  ...
  if (oeeDataList.length === 0) {
    ...
    tbody.innerHTML = `
      <tr>
        <td colspan="${visibleColCount}" class="data-table-centered">
          <div class="fiori-alert fiori-alert--info">
            <strong>ℹ️ Information:</strong> No OEE row data matching the selected conditions.
          </div>
        </td>
      </tr>
    `;
...
function stopTracking() {
  ...
  document.getElementById('connectionStatus').textContent = 'OEE Row Data System Connection Closed';
...
function exportData() {
  ...
  try {
    ...
    // Create download URL
    const exportUrl = `proc/log_oee_row_export.php?${params.toString()}`;
