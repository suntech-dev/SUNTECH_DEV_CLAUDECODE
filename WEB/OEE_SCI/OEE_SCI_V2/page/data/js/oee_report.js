...
document.addEventListener('visibilitychange', () => {
  if (document.hidden) {
    console.log('OEE Report: 페이지 숨김 - SSE 연결 종료');
    ...
    console.log('OEE Report: 페이지 표시 - SSE 재연결 시도');
    if (!isTracking && eventSource === null) {
      reconnectAttempts = 0;
      startAutoTracking();
    }
  }
...
window.addEventListener('pagehide', () => {
  console.log('OEE Report: 페이지 언로드 - SSE 연결 종료');
  if (eventSource) {
    eventSource.close();
    eventSource = null;
    isTracking = false;
...
window.addEventListener('beforeunload', () => {
  console.log('OEE Report: beforeunload - SSE 연결 종료');
  if (eventSource) {
    eventSource.close();
    eventSource = null;
    isTracking = false;
...
function createOeeGradeChart() {
  ...
  return new Chart(ctx, {
    type: 'doughnut',
    data: {
      labels: ['Excellent (≥85%)', 'Good (70-84%)', 'Fair (50-69%)', 'Poor (<50%)'],
      datasets: [{
...
async function startAutoTracking() {
  ...
  try {
    ...
    const sseUrl = `proc/oee_report_stream.php?${params.toString()}`;
...
function updateStatCardsFromAPI(statsData) {
  ...
  const elements = {
    'overallOee': (statsData.overall_oee || '0') + '%',
    'availability': (statsData.availability || '0') + '%',
    'performance': (statsData.performance || '0') + '%',
    'quality': (statsData.quality || '0') + '%',
    'currentShiftOee': (statsData.current_shift_oee || '0') + '%',
    'previousDayOee': (statsData.previous_day_oee || '0') + '%'
  };
...
function updateOeeDetailsFromAPI(oeeDetails) {
  ...
  const elements = {
    'oeeRateDetail': (oeeDetails.overall_oee || '0') + '%',
    'overallEfficiency': (oeeDetails.overall_oee || '0') + '%',
    'targetAchievement': oeeDetails.target_achievement || '-',
    'runtime': oeeDetails.runtime || '-',
    'plannedTime': oeeDetails.planned_time || '-',
    'availabilityDetail': (oeeDetails.availability || '0') + '%',
    'actualOutput': oeeDetails.actual_output || '-',
    'theoreticalOutput': oeeDetails.theoretical_output || '-',
    'performanceDetail': (oeeDetails.performance || '0') + '%',
    'goodProducts': oeeDetails.good_products || '-',
    'defectiveProducts': oeeDetails.defective_products || '-',
    'qualityDetail': (oeeDetails.quality || '0') + '%'
  };
...
function updateOeeTrendChart(oeeTrendStats) {
  if (!charts.oeeTrend || !oeeTrendStats) {
    return;
  }

  try {
    // API에서 {view_type, data} 형태로 반환하므로 data 배열 추출
    const trendData = oeeTrendStats.data || oeeTrendStats;

    if (!Array.isArray(trendData) || trendData.length === 0) {
      return;
    }

    const labels = trendData.map(item => item.time_label || item.display_label);
    const oeeValues = trendData.map(item => parseFloat(item.overall_oee) || 0);
...
function updateProductionTrendChart(productionTrendStats) {
  if (!charts.productionTrend || !productionTrendStats || !Array.isArray(productionTrendStats) || productionTrendStats.length === 0) {
    return;
  }

  try {
    const labels = productionTrendStats.map(item => item.time_label || item.display_label);
    const actualOutput = productionTrendStats.map(item => parseInt(item.actual_output) || 0);
    const targetOutput = productionTrendStats.map(item => parseInt(item.target_output) || 0);
...
function updateMachineOeeChart(machineOeeStats) {
  ...
  try {
    const labels = machineOeeStats.map(item => item.machine_no);
    ...
    console.error('Machine OEE chart update error:', error);
  }
}
...
function populateOeeModal(oeeData) {
  try {
    ...
    setModalValue('modal-overall-oee', (oeeData.overall_oee || 0) + '%');
    setModalValue('modal-availability', (oeeData.availability || 0) + '%');
    setModalValue('modal-performance', (oeeData.performance || 0) + '%');
    setModalValue('modal-quality', (oeeData.quality || 0) + '%');
    ...
    setModalValue('modal-theoretical-output', oeeData.theoretical_output || '-');
